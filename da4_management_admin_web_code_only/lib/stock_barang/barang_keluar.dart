...
class _GudangKeluarScreenState extends State<GudangKeluarScreen> {
  ...
  Future<bool> pickImage(int index, String method) async {
    try {
      ...
      final firestorageURL = await BuktiDataHandlerAndroid.instance
          .uploadImageURL(
              pickedImage, imageName, currentDate, widget.namaGudang);
  ...
  Future<void> checkLaporanExist() async {
    try {
      ...
      barangKeluarStockItems = List<Map<String, dynamic>>.from(
          laporanGudangKeluarData['barangKeluar']);
      //
      canEdit = true;

      if (laporanGudangKeluarData['uploaded'] == true) {
        canEdit = false;
      }
      ...
      setState(() {
        ...
        for (int i = 0;
            i < laporanGudangKeluarData['barangKeluar'].length;
            i++) {}
